...
export default function DashboardPage() {
  ...
  const loadDashboard = () => {
    ...
    getDashboard({ noCache: true })
      .then((r) => {
        ...
        setData(payload)
        // additionally fetch orders meta to get total count
        try {
  ...
  useEffect(() => {
    // Force load on every mount (each visit to /dashboard)
    loadDashboard()
  }, [])
  ...
  const recentOrders = data.recent_orders ?? data.orders ?? []
  // Normalize orders: backend may return array directly or wrap in { data: [...] }
  const orders = Array.isArray(data) ? data : (data.data ?? data.orders ?? data.recent_orders ?? [])

  // Low stock products: support backend returning products array or low_stock list
  const lowStock = (data.low_stock ?? data.products ?? []).filter((p) => Number(p?.stock_quantity || 0) < 10)

  // Orders count: prefer server-provided total (from /orders meta), otherwise fall back to orders length
  const ordersCount = ordersTotal != null ? ordersTotal : (Array.isArray(orders) ? orders.length : 0)

  // Revenue: sum total_amount across all orders
  const revenue = (Array.isArray(orders) ? orders : []).reduce((sum, order) => {
    const amt = parseFloat(order.total_amount) || 0
    return sum + amt
  }, 0)

  // New customers: count unique normalized emails from orders (check both top-level and nested customer.email)
  const newCustomers = (() => {
    ...
    <>
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button className="btn btn-ghost btn-sm" onClick={loadDashboard}>Обновить</button>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:16, marginBottom:24 }}>
        <KpiCard icon={ShoppingCart} label="Заказов всего"    value={ordersCount} color="#3b82f6"/>
        <KpiCard icon={DollarSign}   label="Выручка"          value={revenue.toLocaleString()+' ₽'} color="#22c55e"/>
        <KpiCard icon={Package}      label="Мало на складе"   value={lowStock.length+' товаров'} color="#f59e0b"/>
        <KpiCard icon={Users}        label="Новых клиентов"   value={newCustomers} color="#a855f7"/>
      </div>
